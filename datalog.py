...
class DataLogRecord:
    ...
    def getInteger(self) -> np.int64:
        if len(self.data) != 8:
            raise TypeError("not an integer")
        return np.int64(int.from_bytes(self.data, byteorder="little", signed=True))
  ...
  with open(filename, "r") as f:
    ...
    for record in reader:
        ...
        if record.isStart():
            ...
            if tables.get(entry.name) is None:
              tables[entry.name] = {}
            # print(f"<name='{entry.name}', type='{entry.type}'> [{timestamp}]")

            try:
                # print(f"entry.name: {entry.name:<30} record.entry: {record.entry:<5} record.timestamp: {record.timestamp:<12}")
                ...
                if entry.type == "double":
                    ...
                    tables[entry.name][record.timestamp] = record.getDouble()
                elif entry.type == "int64":
                    # print(f"  {record.getInteger()}")
                    tables[entry.name][record.timestamp] = record.getInteger()
                elif entry.type in ("string", "json"):
                    # print(f"  '{record.getString()}'")
                    tables[entry.name][record.timestamp] = record.getString()
                elif entry.type == "msgpack":
                    # print(f"  '{record.getMsgPack()}'")
                    tables[entry.name][record.timestamp] = record.getMsgPack()
                elif entry.type == "boolean":
                    # print(f"  {record.getBoolean()}")
                    tables[entry.name][record.timestamp] = record.getBoolean()
                elif entry.type == "boolean[]":
                    arr = record.getBooleanArray()
                    # print(f"  {arr}")
                    tables[entry.name][record.timestamp] = arr
                    ...
                    #print(f"  {record.data.hex()}")
                    tables[entry.name][record.timestamp] = record.data
            except TypeError:
                print("  invalid")
    return {'signals': entries, 'values': tables}
# 
